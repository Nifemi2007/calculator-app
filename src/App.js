  ...
  const handlevalue = (inp) => {
    if (inp === "=") {
      try {
        ...
        setValue(result.toFixed(3).toString());
      } catch {
        setValue("Error");
      }
      return;
    ...
    <GlobalStateContext.Provider value={{ dark }}>
      <div className="app" style={{ backgroundColor: dark ? "#252525" : "" }}>
        <main
          style={{
            color: dark ? "#ffac09" : "",
            backgroundColor: dark ? "#060606" : "",
          }}
        >
          {/* display screen */}
          <div
            className="screen"
            style={dark ? { backgroundColor: "#252525", color: "white" } : {}}
          >
            <input
              value={value}
              ref={inputValue}
          ...
          <div className="upper_btn">
            {dark ? (
              <FaToggleOn className="icon" onClick={handleDark}></FaToggleOn>
            ) : (
          ...
          <div className="buttons">
            {buttonText.map((btn) => (
              <Button key={btn} text={btn} func={handlevalue} />
            ))}
            {/* <Button  text={1}  />
      <Button text={2} />
      <Button text={3} />
